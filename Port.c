 ...
 * File Name: Port.c
 *
 * Description: Program file for TM4C123GH6PM Microcontroller - Port Driver.
 *
 * Author: ALMALKY
 * 
 * Date : 3/8/2021
...
/* SET the corresponding bit in the GPIOAMSEL register to Enable analog functionality on this pin */
      case PORT_LEVEL_Analog : SET_BIT(*(volatile uint32*)((volatile uint8*)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , PORT_PTR[i].PORT_PIN);
      break;     
    }   
  ...
  /*
      ...
      if(PORT_PTR[i].PORT_MODE == MODE_DIS_CHANGE) 
    {           
        CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_COMMIT_REG_OFFSET) , PORT_PTR[i].PORT_PIN );  
    }
    else if(PORT_PTR[i].PORT_MODE == MODE_EN_CHANGE)
    {
        *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_LOCK_REG_OFFSET) = 0x4C4F434B;                    
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_COMMIT_REG_OFFSET) , PORT_PTR[i].PORT_PIN ); 
    else
    {
...
* Parameters (out): None
* Return value: None
* Description: Setup the direction of the port pin
************************************************************************************/
...
void Port_SetPinDirection(Port_PinType Pin,Port_PinDirectionType Direction){
	...
	else
	{
        ...
        if (PORT_DIR_DIS_CHANGE == PORT_PTR[Pin].DIRECTION_CHANGEABLE)
	{
...
void Port_SetPinMode( Port_PinType Pin,
	...
	else
	{
        ...
        if (PORT_MODE_DIS_CHANGE == PORT_PTR[Pin].MODE_CHANGEABLE)
	{
...
/* Enable Alternative function for this pin by clear the corresponding bit in GPIOAFSEL register */
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) , PORT_PTR[Pin].PORT_PIN );         
/* ALT_FUNC VALUE the PMCx bits for this pin */
        *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) |= ( Mode << (PORT_PTR[Pin].PORT_PIN)* 4);         
        }   
}
